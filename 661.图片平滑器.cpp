...
{
public:
    int step(int x, int y, string &where, vector<vector<int>> &map)
    { //走路模拟
        if (where == "w")
        {
            ++x;
    ...
    int avg_grayscale(int i, int j, vector<vector<int>> &map)
    {                                                                          //计算平均灰度
...
//这道题有点烦了，直接抄了
// √ Accepted
//   √ 202/202 cases passed (896 ms)
//   √ Your runtime beats 5.84 % of cpp submissions
//   √ Your memory usage beats 5.45 % of cpp submissions (90.7 MB)